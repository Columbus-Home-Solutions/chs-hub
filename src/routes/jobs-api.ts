...
export async function handleJobDetail(env: Env, id: string): Promise<Response> {
  ...
  // Surface conversion_reversed and AI close-out review (Sprint 12 / Sprint 25).
  const reversal = await env.DB.prepare(
    "SELECT conversion_reversed, reversal_reason, reversed_at, ai_closeout_review, ai_closeout_generated_at FROM jobs WHERE id = ?",
  )
    .bind(id)
    .first<{
      ...
      ai_closeout_review: string | null;
  ...
  // Parse AI close-out review JSON blob (stored as text).
  let aiCloseoutReview: Record<string, unknown> | null = null;
  if (reversal?.ai_closeout_review) {
    try {
      aiCloseoutReview = JSON.parse(reversal.ai_closeout_review) as Record<string, unknown>;
    } catch {
      aiCloseoutReview = null;
    }
  }
  ...
  return json({
    job: {
      ...
      ai_closeout_review: aiCloseoutReview,
      ai_closeout_generated_at: reversal?.ai_closeout_generated_at ?? null,
    },
    financial: {
...
// ─── POST /api/jobs/:id/ai-closeout ──────────────────────────────────────────

export async function handleJobAiCloseout(request: Request, env: Env, id: string): Promise<Response> {
  const guarded = await guard(request, env, [...WRITE_ROLES]);
  if (guarded instanceof Response) return guarded;

  const job = await env.DB.prepare(
    "SELECT id, title, job_type, billing_model, contract_total FROM jobs WHERE id = ? AND source = 'estimate'",
  )
    .bind(id)
    .first<{ id: string; title: string | null; job_type: string | null; billing_model: string | null; contract_total: number | null }>();
  if (!job) return err(404, "not_found", "Job not found.");

  const review = await runAiCloseout(env, id, job);
  if (!review) return err(500, "ai_unavailable", "AI close-out analysis failed. Try again.");

  return json({ job_id: id, review, generated_at: new Date().toISOString() });
}

/** Runs Claude close-out analysis and persists result. Safe to call from ctx.waitUntil(). */
async function runAiCloseout(
  env: Env,
  jobId: string,
  job: { title: string | null; job_type: string | null; billing_model: string | null; contract_total: number | null },
): Promise<Record<string, unknown> | null> {
  try {
    // Assemble the job snapshot from D1.
    const invRow = await env.DB.prepare(
      `SELECT
         COALESCE(SUM(CASE WHEN status != 'void' THEN COALESCE(amount, total_due, 0) ELSE 0 END), 0) AS total_invoiced,
         COUNT(CASE WHEN status = 'paid' THEN 1 END) AS invoices_paid_count,
         COUNT(CASE WHEN status NOT IN ('paid','void') THEN 1 END) AS invoices_unpaid_count,
         MAX(CASE WHEN invoice_type = 'final' THEN 1 ELSE 0 END) AS has_final_invoice
       FROM invoices WHERE job_id = ?`,
    ).bind(jobId).first<{ total_invoiced: number; invoices_paid_count: number; invoices_unpaid_count: number; has_final_invoice: number }>();

    const payRow = await env.DB.prepare(
      `SELECT COALESCE(SUM(amount), 0) AS total_paid FROM payments WHERE job_id = ?`,
    ).bind(jobId).first<{ total_paid: number }>();

    const expRow = await env.DB.prepare(
      `SELECT COALESCE(SUM(amount), 0) AS total_expenses, COUNT(*) AS expense_count FROM expenses WHERE job_id = ?`,
    ).bind(jobId).first<{ total_expenses: number; expense_count: number }>();

    const coRow = await env.DB.prepare(
      `SELECT COUNT(CASE WHEN status IN ('draft','sent') THEN 1 END) AS open_change_orders,
              COALESCE(SUM(CASE WHEN status = 'approved' THEN COALESCE(total, 0) ELSE 0 END), 0) AS total_change_order_value
       FROM change_orders WHERE job_id = ?`,
    ).bind(jobId).first<{ open_change_orders: number; total_change_order_value: number }>();

    const taskRow = await env.DB.prepare(
      `SELECT COUNT(*) AS total_tasks,
              COUNT(CASE WHEN status IN ('complete','skipped') THEN 1 END) AS complete_tasks,
              COUNT(CASE WHEN is_punch_list = 1 AND status NOT IN ('complete','skipped') THEN 1 END) AS open_punch_list_items
       FROM tasks WHERE job_id = ?`,
    ).bind(jobId).first<{ total_tasks: number; complete_tasks: number; open_punch_list_items: number }>();

    const snap = {
      title: job.title ?? "Untitled Job",
      job_type: job.job_type ?? "unknown",
      billing_model: job.billing_model ?? "unknown",
      contract_total: job.contract_total ?? 0,
      total_invoiced: invRow?.total_invoiced ?? 0,
      invoices_paid_count: invRow?.invoices_paid_count ?? 0,
      invoices_unpaid_count: invRow?.invoices_unpaid_count ?? 0,
      has_final_invoice: (invRow?.has_final_invoice ?? 0) === 1,
      total_paid: payRow?.total_paid ?? 0,
      total_expenses: expRow?.total_expenses ?? 0,
      expense_count: expRow?.expense_count ?? 0,
      open_change_orders: coRow?.open_change_orders ?? 0,
      total_change_order_value: coRow?.total_change_order_value ?? 0,
      open_punch_list_items: taskRow?.open_punch_list_items ?? 0,
      total_tasks: taskRow?.total_tasks ?? 0,
      complete_tasks: taskRow?.complete_tasks ?? 0,
    };

    const userPrompt = `Job close-out review for: ${snap.title}
Billing model: ${snap.billing_model}
Contract total: $${snap.contract_total}

Financial snapshot:
- Total invoiced: $${snap.total_invoiced} (${snap.invoices_paid_count} paid, ${snap.invoices_unpaid_count} unpaid)
- Total received: $${snap.total_paid}
- Total expenses logged: $${snap.total_expenses} (${snap.expense_count} expense records)
- Has final invoice: ${snap.has_final_invoice}
- Open change orders: ${snap.open_change_orders}
- Approved change order value: $${snap.total_change_order_value}

Task snapshot:
- ${snap.complete_tasks} of ${snap.total_tasks} tasks complete
- Open punch list items: ${snap.open_punch_list_items}

Produce a JSON object with this exact shape:
{
  "summary": "2-3 sentence plain-language summary of job close-out status",
  "flags": [
    {
...
revenue_gap = contract_total - total_paid, set to null if zero or negative.
expense_gap_note: if expense_count < 3 or total_expenses < (contract_total * 0.3), note it. Otherwise null.
Only include flags that are actually true. Keep flags short and specific.`;

    const result = await claudeMessages(env, {
      system:
        "You are a construction business analyst reviewing a job close-out for Columbus Home Solutions, a residential remodeling contractor. Analyze the job data and produce a concise close-out review in JSON format only. Be specific with numbers. Flag genuine issues, not hypothetical ones. Return ONLY valid JSON — no markdown, no preamble.",
    ...
    await env.DB.prepare(
      "UPDATE jobs SET ai_closeout_review = ?, ai_closeout_generated_at = ? WHERE id = ?",
    )
      .bind(JSON.stringify(parsed), nowIso, jobId)
      .run();

    return parsed;
  } catch (e) {
    console.error("[ai-closeout] failed:", (e as Error).message);
    return null;
  }
}
...
export async function handleJobStatus(request: Request, env: Env, id: string, ctx: ExecutionContext): Promise<Response> {
  ...
  if (target === "complete") {
    ...
    // Sprint 25 D2C: fire AI close-out analysis in background; response returns immediately.
    const jobForAi = await env.DB.prepare(
      "SELECT title, job_type, billing_model, contract_total FROM jobs WHERE id = ?",
    ).bind(id).first<{ title: string | null; job_type: string | null; billing_model: string | null; contract_total: number | null }>();
    if (jobForAi) {
      ctx.waitUntil(
        runAiCloseout(env, id, jobForAi).catch((e) =>
          console.error("[jobs-api] ai-closeout (complete) failed:", (e as Error).message),
        ),
      );
    }
  }

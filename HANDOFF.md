...
**Post-deploy infrastructure fix (Jun 3, 2026):** Cloudflare has a **GitHub integration** auto-deploying the worker on every `git push` — it was running `wrangler deploy` directly, which skipped the Vite build and stripped all `/app/` static assets (gitignored build output). Fix: added `[build] command = "bash scripts/build-frontend.sh"` to `wrangler.toml` and extracted `scripts/build-frontend.sh` (Vite build + build-timestamp injection). `npm run deploy` now simply calls `wrangler deploy` (build handled by wrangler.toml). Future pushes trigger the integration but it now runs the build first. **Action item for Tony:** check Cloudflare dashboard → Workers & Pages → chs-hub → Settings → Integrations and confirm the deploy command is correct (ideally `npm run deploy`).

**Browser-verified by Tony (Jun 3, 2026 — S18 feature pass):**
- ✅ Photo annotation — draw markup, save → annotation overlay visible on photo; ✏️ badge shows on thumbnail
- ✅ Before/after type labeling — "Before"/"After" labels appear on thumbnails
- ✅ Photo Report generated → appeared in Documents tab as `photo_report` category, 4 KB, PENDING sync
- ✅ Project Packet generated → appeared in Documents tab as `project_packet` category, 3 KB, PENDING sync, distinct from completion package
- ✅ Edit markup / Show original / Before/After buttons all present in photo detail modal
- ✅ All prior features (jobs, invoices, documents, portal, Settings) unaffected

**Post-verification UX fixes (commit `3554860`, same deploy session):**
- Annotation ✏️ badge: also renders when `annotation_data` is non-null (guards async refetch timing gap)
- Before/After slider: now also renders when viewing the **Before** photo (previously only rendered on the After side); `pairedAfter` lookup added
- Unpair: works from either side of a pair
- Pairing picker instruction text rewritten to be unambiguous

**Verified in production (curl + remote DB):** `/api/health/heartbeat` → `{"status":"ok","tables":41,"tables_expected":41,"settings":44}`; `device_tokens` present, both indexes, 0 rows; `social_publish_mode=SIMULATE`; 5 crons; `NOTIFICATIONS_DISPATCH_MODE=simulate`.

**The one schema change:** `migrations/0039_device_tokens.sql` — **NEW TABLE `device_tokens`** (`id` PK, `user_id`→users, `platform` CHECK `ios|android|web`, `token`, `is_active`, `created_at`, `last_seen_at`; UNIQUE(token); idx(user_id,is_active)). The **only** DDL this sprint — applied locally by direct-execute (`d1 execute --local --file`). Everything else is pure code over existing columns: `photos.is_annotated` / `annotation_data` / `before_after_pair_id` (from `0031`) and free-text `documents.document_category` (`photo_report` + new `project_packet`).

**Photo features (the three S8-deferred seams, now built):**
- **Annotations** — `PUT /api/photos/:id/annotate` + `GET /api/photos/:id/annotation`. **Non-destructive** (business rule 1): saves a validated overlay to `annotation_data`, sets `is_annotated=1`, **never rewrites the R2 original**; the annotated view is composited from original + overlay. The JSON↔render contract lives in **one place** — `src/lib/annotation.ts` (mirrored client-side at `frontend/src/lib/annotation.ts`): shapes (pen/rect/ellipse/arrow/text/crop) in **absolute natural-pixel coords**, rendered to a self-contained SVG (`<image>` + overlay, crop = viewBox change) so the web view and the HTML report composite **identically**. Canvas editor: `frontend/src/views/jobs/PhotoAnnotator.tsx`. Round-trip is lossless (tested).
- **Before/after pairing** — `POST /api/photos/pair` (`{before_id, after_id}`; direction: after→before, sets before/after type flags so the existing portal Photos tab + completion-package before/after queries surface it — not rebuilt) + `POST /api/photos/unpair`. Comparison slider: `frontend/src/views/jobs/BeforeAfterSlider.tsx` (clip-path divider).
- **Photo report** — `POST /api/jobs/:id/photo-report` (O/PM). HTML-first (S15 pattern, **no Puppeteer/binary-PDF**): branded printable HTML → R2 → `documents` row `document_category='photo_report'` (`is_signed=0`, kept out of any unscoped signed-doc query). Options `{photo_ids, include_gps, include_captions}`; annotated photos composited in; shareable via the existing `/api/documents/:id/share` → `/share/:token`. Builder UI: `frontend/src/views/jobs/PhotoReportBuilder.tsx`.
- **Project packet** — `POST /api/jobs/:id/project-packet` (O/PM). Lean sales artifact (before/after prioritized + scope from `jobs.notes` + social-ready highlights), HTML-first → `documents` row `document_category='project_packet'`, **distinct from the completion package** (no financials/warranty/doc inventory).

**Push (SIMULATE):** `POST /api/devices/register` / `unregister` + `GET /api/devices` (`src/routes/devices.ts`, **ALL** roles — every user registers their own device; tokens **masked** to last-4 in all responses/logs/audit, mirroring the S17 token rule). The dispatcher's `push` branch (`src/lib/notification-engine.ts` `sendByPush`) resolves the recipient's active `device_tokens` and **logs the intended push (masked) — no live FCM/APNS**. On the native shell, `frontend/src/app.tsx` calls `registerPushDevice()`; on web it's a no-op (PWA unchanged).

**Capacitor (config + checklist; native build is Tony's out-of-band step):** `capacitor.config.ts` (appId `com.homesolutionsar.chs`, **bundled assets** `webDir: public/app` — decision recorded), `frontend/src/lib/native.ts` (runtime detection + push/haptics via the global `Capacitor` bridge, **graceful web fallback, no @capacitor npm dep in the web bundle**), `scripts/capacitor-setup.sh` (installs the toolchain + adds `ios/`+`android/`), `public/docs/CHS-Capacitor-Submission-Checklist.md`. **⚠️ Recorded Pre-Launch blocker:** a native WebView won't carry a Cloudflare Access session to the Access-gated API host — must be resolved (service/device-auth, a public API origin, or in-WebView Access login) before the native app can authenticate; intersects the unresolved `APP_PUBLIC_ORIGIN`/public-hostname item.

**Hardening:** Fixed-element CSS audit (S11 lesson) — aligned two off-scale `z-index:50` rules to the token scale (`.notif-dropdown`→`--z-dropdown`; `.qc-uploading`→ above the capture bar/nav). Perf: no rewrite needed; the photo-report query uses an indexed `IN (…)` over a small per-job set.

**Tests/typecheck:** worker `tsc` clean, frontend `tsc` clean, frontend Vite build clean, **94/94 vitest** (new: `tests/photo-features.test.ts`, `tests/photo-lifecycle.test.ts`, +RBAC matrix for the new routes). Verified live against the local dev server (annotate round-trip, pair persistence, report+packet generation with correct categories + `is_signed=0`, device register masking).

**🟡 Open items carried forward:** (1) **Full cross-module automated E2E** harness (lead→estimate→quote/deposit→job→notifications→photos→invoice→payment→completion→social) — not yet committed (data-layer photo-lifecycle test is the down payment; follow-on Cursor task to run against prod). (2) **Native iOS/Android build + store submission** (Tony, per `public/docs/CHS-Capacitor-Submission-Checklist.md`). (3) **⚠️ Pre-Launch blocker:** native WebView can't carry a Cloudflare Access session — must resolve before Capacitor app can authenticate (intersects `APP_PUBLIC_ORIGIN`/public-hostname). (4) `weekly_photo_summary` / `CurrencyInput` deferred (no trigger met). (5) Photo Report and Project Packet show `PENDING` sync status in Documents — expected (they are R2 + DB rows, not synced to Drive; no action needed unless Drive sync is desired).

**Key new files:** `migrations/0039_device_tokens.sql`, `src/lib/annotation.ts`, `src/lib/photo-report.ts`, `src/lib/project-packet.ts`, `src/routes/devices.ts`, `src/routes/photo-report.ts`, `src/routes/project-packet.ts`, `capacitor.config.ts`, `scripts/capacitor-setup.sh`, `public/docs/CHS-Capacitor-Submission-Checklist.md`, `frontend/src/lib/{annotation,native}.ts`, `frontend/src/views/jobs/{PhotoAnnotator,BeforeAfterSlider,PhotoReportBuilder}.tsx`, `tests/{photo-features,photo-lifecycle}.test.ts`. Extended: `src/index.ts` (device + annotate/annotation + pair/unpair + photo-report + project-packet routes), `src/routes/photos.ts` (annotate built out, pair/unpair, annotation_data/before_after_pair_id surfaced), `src/lib/notification-engine.ts` (`sendByPush` SIMULATE), `src/lib/rbac.ts` (O/PM rules for report/packet), `src/routes/documents.ts` (+`project_packet` category), `src/lib/completion-package.ts` (exclude `project_packet`), `frontend/src/{app.tsx,views/jobs/PhotosTab.tsx,styles/app.css}`, docs (`CHS-API-Route-Map.md` §6 as-built + `/api/devices/*`; `CHS-Database-Schema.md` `0039`).

### Shipped — Sprint 17 (Jun 2, 2026) — System Admin & Multi-User Foundation

**Tags:** `v0.17.0-sprint17` (code on `main`, commit **`b35b891`** — `fix(sprint17): align 0038 prod-parity seed ids to actual prod ids`; the feature commit is **`eebba95`** — `feat(sprint17): system admin & multi-user foundation …`), `v0.17.0-sprint17-deployed` (production infra).

**Production:** Worker version **`df9b1fb8-a573-431b-9446-6c9cd270a9d3`** on `https://chs-hub.tony-bc5.workers.dev` + `dashboard.homesolutionsar.com` + `client.homesolutionsar.com`. Migration **`migrations/0038_prod_parity_seed.sql`** applied remote by **direct execute** (NOT `migrations apply` — same unrecorded-ledger reason as prior sprints). **`0038` is DATA-ONLY (no schema change): nine `INSERT OR IGNORE` rows** — 5 `document_templates` (`tpl-s15-*`), 1 `notification_templates` (`nt-completion-package-sent-email`, `trigger_event='completion_package_sent'`), 3 social `system_settings`. **Verified true no-op on prod** (counts before==after: document_templates 6, notification_templates 32, social settings 3; zero `seed-%` ids landed). Backup before remote writes: **`backup_pre_sprint17_remote_20260602.sql`** (local, gitignored) + R2 **`chs-backups/`**. Rollback tag if needed: **`v0.16.0-sprint16-deployed`** (0038 is INSERT OR IGNORE only — no schema/destructive writes, so a code rollback needs no DB action). **Stripe test / notifications SIMULATE / QBO sandbox / WC test sheet / `social_publish_mode=SIMULATE` — nothing flipped. Cron count still 5 (RBAC added no cron).**

**The single most important fact:** **RBAC is now enforced in production.** A centralized `enforceRbac` gate (`src/lib/rbac.ts`, called in `src/index.ts` before route dispatch) authorizes every gated `/api` route by role; PUBLIC/token/webhook/secret routes (pay/quote/portal/share/webhooks/ops/sync/hl) are explicitly exempt. The **owner passes every gate** (business rule 11 preserved) and **Tony is the only active user** (`role=owner`, `is_active=1`), so there is zero change to Tony's workflow — but the gate is real: a non-owner/inactive identity now gets 403/401. Auth model unchanged (resolves `Cf-Access-Authenticated-User-Email` → active `users` row); RBAC sits *after* resolution. Frontend mirrors the capability model (`frontend/src/lib/rbac.ts`) to gate nav/tabs/buttons. **Settings is now a tabbed, owner-only page** (Company, Financial, Integrations, Notifications, Users, Audit, DLQ, Backup, Health) surfacing existing infra — user CRUD+invite+per-user notification prefs, audit-log viewer + CSV export, DLQ list/retry/dismiss over `sync_dead_letters`, backup status + manual trigger, system-health panel, integration connection cards.

**⚠️ Deploy-time correction (important provenance):** the committed `0038` was first reconstructed from the **local** DB, which used `seed-`-prefixed ids (`seed-tpl-s15-*`, `seed-ntpl-s15-cp`). Step-3 pre-flight (remote `PRAGMA`/`SELECT`) revealed **prod uses different ids** — `tpl-s15-*` and `nt-completion-package-sent-email` (the S15 hand-inserts, see the Sprint 15 entry below). Because `document_templates`/`notification_templates` have **no UNIQUE constraint beyond the PK**, running the original would have `INSERT`ed **6 duplicate rows** on prod instead of being a no-op. Fixed by aligning all 6 ids to the verified prod ids (commit `b35b891`), re-ran tests (76/76), re-committed, moved the `v0.17.0-sprint17` tag, then executed — confirmed no-op. **Lesson: prod-parity seeds must be keyed from prod truth, not the local DB.**

**Verified in production (curl + remote DB, Jun 2, 2026):** deploy healthy (version `df9b1fb8`, 5 crons listed); `/api/health/heartbeat` 200 (40/40 tables, 44 settings); `0038` true no-op (counts unchanged, no dup `completion_package_sent`, no `seed-%` ids); **unauth `/api/users` → 401**, **public `/api/public/pay/:token` → 404 (reachable, not gated)**; client-host routing guard intact (`/app` 404, portal/pay bad-token → 404 not 500); `social_publish_mode` still `SIMULATE`. **The owner→200 and PM/field-crew/office-admin→403 matrix is covered by the 76 local tests** — it cannot be curled on prod because Cloudflare strips client-supplied `Cf-*` headers at the edge (the real `Cf-Access-Authenticated-User-Email` is only injected by Access on the gated host), which is itself a good anti-spoofing property.

**🟡 Open item — owner browser pass (6a) not yet done:** the Settings UI E2E (tabs render for owner; Company/Financial edit→save→audit entry; Users add-user form; Audit CSV export; **Backup "Trigger Now" → new `backups/d1/…` R2 object**; Health shows 5 crons + integration statuses; Integrations test buttons) requires Tony's logged-in Access session and was **not** performed in this automated deploy. Recommend Tony run that pass next session in a browser on `dashboard.homesolutionsar.com`.

**Resolves prior carry-forwards:** the long-standing **"hand-inserted prod rows owed a migration"** (S15 templates + completion template + S16 social settings) is now committed as `0038`; the **`completion_package_sent` vs `job_completion_package` naming divergence** is reconciled (engine treats `completion_package_sent` as transactional; the old key retained only as a deprecated alias; prod's spec row `nt-job-completion-package-email` remains but is dead for this path).

**Key new files:** `src/lib/rbac.ts` (capability model + route→role table + `enforceRbac`), `src/lib/audit.ts` (shared audit writer), `src/routes/{users,audit-logs,dlq,backup,health}.ts`, `migrations/0038_prod_parity_seed.sql`, `frontend/src/lib/rbac.ts`, `frontend/src/views/settings/{UsersTab,ReliabilityTabs,SettingsEditors,IntegrationsTab}.tsx`, `tests/{rbac,system-admin}.test.ts`. Extended: `src/index.ts` (enforceRbac gate + new routes), `src/middleware/roles.ts` (`requireOwner`), `src/lib/ops/dlq.ts` (`retryDeadLetter`/`dismissDeadLetter`), `src/routes/integrations.ts` (generic test/connect/disconnect), `src/lib/notification-engine.ts` (`completion_package_sent` transactional), `frontend/src/{components/layout/Sidebar.tsx,lib/nav.ts,views/settings/Settings.tsx}`, docs (`CHS-API-Route-Map.md` cron table → 5 as-built + admin routes; `CHS-Seed-Data.md`; `CHS-Database-Schema.md` 0038 log). **Local seed (gitignored):** `scripts/dev-seed-sprint17.local.sql` (multi-user fixture: PM/field-crew/office-admin + deactivated user + sample audit_logs + sync_dead_letters).

**Scope boundary (per the sprint prompt):** Capacitor/App Store work is **Sprint 18** (a "Mobile App (Sprint 18)" placeholder tab exists in Settings). No new external integrations, no new data model beyond the `0038` seed, no new cron jobs, no business-logic changes for non-owner roles. Owner-facing `/app` only.

### Shipped — Sprint 16 (Jun 2, 2026) — Social Media Engine

**Tags:** `v0.16.0-sprint16` (code on `main`, commit **`b8bbe2f`** — `feat(sprint16): social media engine …`), `v0.16.0-sprint16-deployed` (production infra — points at the post-deploy bugfix commit `fix(sprint16): make + New post work — allow empty-caption draft on manual create`).

**Production:** Worker version **`04c0b58d-4213-4307-8c17-7a28ef78a7b5`** (final, includes the + New post fix) on `https://chs-hub.tony-bc5.workers.dev` + `dashboard.homesolutionsar.com` + `client.homesolutionsar.com`. (Deploy sequence: **`584716e1`** initial → **`04c0b58d`** empty-caption fix.) **NO migration** — `social_posts` / `content_schedules` / `v_content_schedule_counts` already existed from `0019`; Sprint 16 added zero schema. Backup before remote writes: **`backup_pre_sprint16_remote_20260602.sql`** (local, gitignored) + R2 **`chs-backups/`**. Rollback tag if needed: **`v0.15.0-sprint15-deployed`** (no schema shipped → a code rollback needs no schema rollback; the only remote writes were 3 additive `system_settings` rows, harmless to the prior Worker). **Stripe test / notifications SIMULATE / QBO sandbox / WC test sheet / `social_publish_mode=SIMULATE` — nothing flipped. Cron count still 5 (publisher folded into the existing `*/15` handler, no new trigger).**

**The single most important fact:** the platform now has a native, AI-assisted social engine that replaces Metricool — but **publishing ships in SIMULATE** (`social_publish_mode=SIMULATE` in `system_settings`; the `*/15` sweep stamps synthetic `facebook_post_id`/`instagram_post_id` and marks posts `published` without calling the Graph API). **Caption + schedule generation runs LIVE Claude** (reuses `src/lib/claude.ts` / existing `ANTHROPIC_API_KEY`, no simulate gate — same as receipts/smart-notes since S8), so generating content on prod is real, billable spend, owner-facing only. **AI image generation stays unconfigured** (no `REPLICATE_API_KEY`) → image actions return the "configure image API" state, zero image spend. All social surfaces are **owner-facing `/app/social` only** — the public-host allowlist (`src/index.ts`) 404s `/app/social`, `/api/social-posts*`, `/api/content-schedules*`, and the image route on `client.homesolutionsar.com` (verified by curl).

**Remote data steps performed this session (provenance — fold into a future migration so a fresh DB build matches prod):**
- **3 `system_settings` rows hand-inserted** on prod (via `scripts/social-settings-remote.local.sql`, gitignored, run with `d1 execute --remote --file`): `social_brand_voice` (`string`, the brand-voice system prompt — identical to the in-code `DEFAULT_BRAND_VOICE` fallback), `social_publish_mode` (`string` = `SIMULATE`), `social_image_gen_count` (`json` = `{}`). **Deliberately left unset:** `social_replicate_api_key`, `social_facebook_page_id`, `social_facebook_page_token`, `social_instagram_account_id` — that's what keeps image-gen unconfigured and publishing in SIMULATE. Same follow-up class as S15's completion-package template + the 5 document templates; bundle into one "prod-parity seed" migration (candidate for S17). `value_type` CHECK is `string|number|boolean|json` — brand voice MUST be `string`.

**Post-deploy bugfix (this session):** the **+ New post** button was dead — it POSTed an empty caption, but `handleSocialPostCreate` required a non-empty caption (400) and the frontend swallowed the error. Fix (`src/routes/social-posts.ts` + `frontend/src/views/social/SocialMedia.tsx`): a **draft may be created with an empty caption** (owner writes/AI-generates it in the editor), **approval now enforces a non-empty caption** (`approveOne` → 400 `empty_caption`) so an empty post can never publish, and the button **surfaces failures via toast**. Redeployed as `04c0b58d`.

**Verified in production (full E2E in a logged-in browser, Jun 2, 2026):** manual post create → inline caption/hashtag edit → approve ✅; **caption regeneration (real Claude, brand voice, city-only / no street address)** ✅; **monthly schedule generation (real Claude → balanced `pending_approval` batch + `active` content_schedules row + colour-coded calendar)** ✅; image-gen gate returns "configure image API" (no Replicate key) ✅; **publish in SIMULATE → synthetic FB/IG ids, no real post, idempotent re-tick** ✅; calendar/queue/editor/history UI + batch approve + mobile swipe ✅. Curl smokes: deploy healthy, 5 crons listed, public-host guard 404s all social surfaces. **Note: `social_ready=0` on prod** — no photos are flagged social-ready yet, so job-completion auto-gen has no material until photos are flagged (the seam fires the SIMULATE "flag photos?" notification instead). **No open Sprint 16 items.**

**Key new files:** `src/lib/social.ts` (shared helpers + `system_settings` getters + audit + photo refs), `src/lib/social-ai.ts` (Claude caption/hashtag, fallback hashtags, no-street-address rule), `src/lib/content-schedule.ts` (balanced monthly planner — the "Opus" piece), `src/lib/social-publish.ts` (publish state machine, IG two-step, 3× backoff via `dead_letter_queue`, `publishDuePosts` cron sweep), `src/lib/image-gen.ts` (key-gated Replicate/Flux Pro + monthly counter), `src/lib/social-jobs.ts` (job→Complete auto-gen seam, `generated_by='ai_job_complete'`), `src/routes/social-posts.ts`, `src/routes/content-schedules.ts`, `src/routes/social-publish.ts`, `frontend/src/views/social/{SocialMedia,ContentCalendar,ApprovalQueue,PublishedHistory,PostEditor}.tsx`, `tests/social-media.test.ts` (18 tests). Extended: `src/index.ts` (social routes + `publishDuePosts` folded into `runNotificationProcessor`), `src/routes/jobs-api.ts` (job→Complete seam), `src/env.ts` (`SOCIAL_PUBLISH_MODE`, `REPLICATE_API_KEY`), `frontend/src/{router.tsx,lib/nav.ts,types.ts,styles/app.css,styles/components.css}`, `public/docs/CHS-API-Route-Map.md` (Social 10→16, approve-batch + image route, `*/15` fold). **Local seed (gitignored):** `scripts/dev-seed-sprint16.local.sql` (settings + social-ready photos + sample posts + active schedule); remote settings via `scripts/social-settings-remote.local.sql`.

**Delete policy note:** `social_posts` has **no `is_active` column**, so unpublished posts **hard-delete** and **published posts are delete-blocked** (decision, no schema change).

**🔴 New carry-forward — live publishing needs PUBLIC image URLs.** Real FB/IG publishing (and the IG container step) fetches the post image by URL; the social image-serve route (`/api/social-posts/:id/image`) is Access-gated today. Flipping `social_publish_mode=live` at Pre-Launch is gated on the **same unresolved `APP_PUBLIC_ORIGIN`** as pay/quote/portal/share — the image route must be reachable on the public host. Added to the Pre-Launch checklist under the existing public-origin blocker.

**Deferred (explicit):** **Google Reviews sync** (spec §7.1 / Build-Order item 11) — NOT built. Blocked on (1) a `saved_reviews` ALTER (`google_review_id`, `reviewer_photo_url`, `reply_text`, `synced_at` — optional `0038`) and (2) **GBP API access** (verified profile 60+ days + a formal Google Cloud request, weeks of lead time, **not yet requested** — request now as a long-lead item). `review_highlight` posts are manual-only until then. Also: the Build-Order claim "Google Reviews bulk import done in Sprint 5" is **false** (S5 was Quote Delivery + Stripe) — correct when convenient.

**Carry-forwards (unchanged):** 🔴 `APP_PUBLIC_ORIGIN` still unresolved (now also gates live social image fetch); QBO crash-window hardening; WC trailing-space tab name; quote-delivery → `document_templates` consolidation (post-attorney-review); S15 notification-template naming divergence (`completion_package_sent` vs `job_completion_package`); the hand-inserted prod rows owed a migration.

### Shipped — Sprint 15 (Jun 2, 2026) — Document Management & Templates

**Tags:** `v0.15.0-sprint15` (code on `main`, commit **`469576a`**), `v0.15.0-sprint15-deployed` (production infra, commit **`7093b1d`** — the post-deploy UX follow-ups). Core feature commit `469576a` (`feat(sprint15): Document Management & Templates`); follow-up `7093b1d` (`feat(sprint15): job-profile Documents tab + Hub Files job dropdown`).

**Production:** Worker version **`f9edcdf9-2254-43ca-8ad5-aa9a3632a1a5`** (final) on `https://chs-hub.tony-bc5.workers.dev` + `dashboard.homesolutionsar.com` + `client.homesolutionsar.com`. (Deploy sequence this session: `aa4eab9e` initial → `688f2890` job dropdown → `7a557fa2` job Documents tab → `f9edcdf9` preview-contrast fix.) Migration **`migrations/0037_document_template_versioning.sql`** applied remote by **direct execute** (NOT `migrations apply` — same unrecorded-ledger reason as prior sprints). It is the **only additive schema change**: `document_templates.previous_version_id TEXT` + partial index `idx_doc_templates_lineage` (documents / lien_waivers needed zero new columns). Backup before migrate: **`backup_pre_sprint15_remote_20260602.sql`** (local, gitignored) + R2 **`chs-backups/`**. Rollback tag if needed: **`v0.14.0-sprint14-deployed`** (0037 is one nullable column + index; the legacy backfill only *added* `documents` rows — both harmless to the Sprint 14 Worker, so a code rollback needs no schema rollback). **Stripe test / notifications SIMULATE / QBO sandbox / WC test sheet — nothing flipped. Cron count still 5 (no new trigger).**

**The single most important fact:** all files across the platform now live in one **`documents`** table (R2 canonical), browsable in the dashboard **Hub Files** view and per-job in the new **job Documents tab**. Document **templates** are DB-backed with **copy-on-write versioning** (edit content → new row, `version+1`, `previous_version_id` → superseded row, old head deactivated; list shows only current heads). The **Job Completion Package** compiles cross-module data (docs + before/after photos + financial summary) into a **branded HTML package** (HTML-first, Workers-compatible — no Node PDF deps) with a **draft → send** state machine encoded schema-free on `documents.is_signed`/`signed_date` (0=draft, 1=sent). The hourly `15 * * * *` Drive mirror was **extended to the `documents` table** (no new cron). Public **`/share/:token`** links are token-gated on the public host. `src/lib/contracts.ts` quote-delivery path was **not** rewired.

**Remote data steps performed this session (provenance — fold into a future migration so a fresh DB build matches prod):**
- **Legacy backfill** (`scripts/migrate-legacy-files-to-documents.sql`, direct execute, idempotent): migrated **2 `company_documents`** rows → `documents` (`context_type='company'`: "Logo", "house plans"); **0 `files`** rows (none on prod). `documents` 0 → 2. Re-run wrote 0 (idempotent confirmed live). Legacy tables retained as archive.
- **Notification template hand-inserted:** `nt-completion-package-sent-email` (`trigger_event='completion_package_sent'`, client/email, `{{client_name}}/{{job_title}}/{{portal_link}}`). ⚠️ **Naming divergence:** prod already had `nt-job-completion-package-email` (`trigger_event='job_completion_package'`, body uses `{{completion_package_link}}`) — the **spec** name. The **code fires `completion_package_sent`** and the engine only populates `{{portal_link}}` (from the job's `portal_token`), so the spec row is dead for this path. Reconcile to one name later (and drop/repoint the unused row).
- **5 `document_templates` hand-inserted** on prod (ids `tpl-s15-*`, all active v1): Service Agreement, Cost-Plus Billing Agreement, Change Order, Lien Waiver, Warranty Certificate. Content mirrors `contracts.ts`. (Prod had **0** templates before this; the rich set lived only in the local-only seed.) **Templates only — no seed test fixtures touched prod.**

**Verified in production (full E2E in a logged-in browser, Jun 2, 2026):** template versioning (edit → v2, old head deactivated, prior generated doc keeps original content) ✅; generate-from-template (preview + generate into Documents, merge fields populated) ✅; lien-waiver create → generate document into `documents` (`document_category='lien_waiver'`, links `document_id`) ✅; **completion package compile (draft) → preview HTML → send (draft→sent) → `notification_logs` SIMULATE row, no real email → resend idempotent** ✅; portal Documents grouped (internal categories hidden) + Completion tab appears **only after send** ✅. Curl smokes: `/health` 200, `/app/documents` 200 (manifest fresh), public-host guard 404s internal, `/share/bogus-token` → **410 handler** (routed on public host, not catch-all). **No open Sprint 15 items.**

**Key new files:** `src/routes/documents.ts`, `src/routes/document-templates.ts`, `src/routes/lien-waivers.ts`, `src/routes/completion-package.ts`, `src/lib/merge-fields.ts`, `src/lib/completion-package.ts`, `migrations/0037_document_template_versioning.sql`, `scripts/migrate-legacy-files-to-documents.sql`, `frontend/src/views/documents/HubFiles.tsx`, `frontend/src/views/settings/DocumentTemplates.tsx`, `frontend/src/views/jobs/DocumentsTab.tsx`. Extended: `src/index.ts` (routes + `/share/*` host allowlist + drive-mirror log), `src/lib/ops/drive-mirror.ts` (documents fold), `src/routes/portal.ts` (grouped docs + completion tab flag), `frontend/src/views/jobs/JobDetail.tsx` (Documents tab), portal frontend (SeamTabs/PortalApp/portalApi), `frontend/src/lib/nav.ts` (Documents nav enabled). **Local seed (gitignored):** `scripts/dev-seed-sprint15.local.sql` (5 templates, sample docs w/ live+expired share tokens, lien waivers, completion draft, the `completion_package_sent` template).

**⚠️ Watch item — `documents.is_signed`/`signed_date` overloaded** for completion-package draft(0)/sent(1) state (no schema change, per the deviation). Any future **global** `WHERE is_signed=1` query (a "signed contracts" filter, a Hub Files "signed" badge) will **also match sent completion packages** — scope such queries by `document_category` or exclude `completion_package`.

**Carry-forwards (unchanged from prior sprints):** 🔴 **`APP_PUBLIC_ORIGIN`** still unresolved — share links + completion-package client links join the pay/quote/portal Pre-Launch blocker (built + verified, but the public URL rides `client.homesolutionsar.com` until the public host is finalized). QBO crash-window hardening; WC trailing-space tab name; quote-delivery → `document_templates` consolidation deferred to post-attorney-review.

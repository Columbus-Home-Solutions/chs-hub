-- Google LSA dedicated Twilio number: allow source='google_lsa' on estimate_requests.
-- Same CHECK-rebuild pattern as 0098 (thumbtack) / 0104 (nullable client_id).
-- Parking notification_logs.estimate_request_id during DROP+RENAME (only FK).

PRAGMA defer_foreign_keys = ON;

CREATE TABLE _er_notif_backup_0121 AS
...
SET estimate_request_id = (
  SELECT b.estimate_request_id FROM _er_notif_backup_0121 b WHERE b.id = notification_logs.id
)
WHERE id IN (SELECT id FROM _er_notif_backup_0121);

DROP TABLE _er_notif_backup_0121;
...
VALUES (
  'google_lsa_tracking_number',
  '',
  'string',
  'integrations',
  'Google LSA tracking number',
  'Dedicated Twilio number Google LSA forwards into. Never use as outbound caller ID — always present 501-263-2050 instead.',
  datetime('now')
)

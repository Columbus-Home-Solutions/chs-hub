 ...
 *
 * Jobber write-back has been decommissioned (Sprint 36). The
 * pushed_to_jobber_at and jobber_id columns remain in the DB as historical
 * data but no new pushes are made.
 */

import type { Env } from "../env.js";
import { expenseTypeAllowsVendorLink } from "../../shared/expense-types.js";
...
export async function handleExpenseCreateJson(env: Env, request: Request, ctx: ExecutionContext): Promise<Response> {
  ...
  // labor + subcontractor both may link a subcontractors-table person (1099 / QBO Vendor).
  const allowsVendorLink = expenseTypeAllowsVendorLink(expenseType);
  const subId = allowsVendorLink ? strv(body.sub_id) : null;
  const is1099 = allowsVendorLink && Boolean(body.is_1099_reportable);
  ...
  // Sprint 20 — Auto-generate Sub Unconditional Lien Waiver for subcontractor expenses.
  // Only fires when expense_type='subcontractor' and sub_id is set and there's a job.
  // Non-blocking: failure must never roll back the expense insert.
  if (expenseType === "subcontractor" && subId && jobId) {
    const paymentDateStr = strv(body.incurred_date) ?? new Date().toISOString().slice(0, 10);
    ctx.waitUntil(
      maybeAutoGenerateDocument(env, {
        jobId,
        templateType: "lien_waiver_sub_unconditional",
        triggerEvent: "sub_payment",
        relatedRecordId: id,
        overrides: {
          sub_id: subId,
        ...
        console.error(
          "[expenses] doc auto-gen (lien_waiver_sub_unconditional) failed:",
          (e as Error).message,
        ),
      ),
    );
...
// ────────────────────────────────────────────────────────────────────────
// POST /api/expenses/:id/push-to-jobber — decommissioned (Sprint 36)
export async function handleExpensePush(
  _env: Env,
  _expenseId: string,
): Promise<Response> {
  return jsonResponse(
    { ok: false, error: "Jobber sync has been decommissioned." },
    { status: 410 },
  );
}

...
1. **⚠️ Email dispatch gate caveat (fix before Pre-Launch).** The runbook assumed email "stays simulated in `live` mode until `NOTIFICATIONS_EMAIL_FROM` is set." **This is NOT true in code:** `sendByChannel` falls back to `env.ALERT_EMAIL_FROM` (which IS set in prod) when `NOTIFICATIONS_EMAIL_FROM` is absent, and `RESEND_DRY_RUN="0"`. So once `NOTIFICATIONS_DISPATCH_MODE="live"`, **email would send via the ALERT_EMAIL_FROM fallback even without `NOTIFICATIONS_EMAIL_FROM`**. The `simulate` flag is currently the *sole* lock on client email. Fix: either drop the `ALERT_EMAIL_FROM` fallback in the email path (`src/lib/notification-engine.ts` ~line 620) or treat the dispatch flag as the only email gate and document it.

...
### Built local-only — Sprint 18 (Jun 2, 2026) — Capacitor App & Polish (awaiting remote deploy)

**Status:** Built and committed to the S17 codebase, **local-only**. Remote deploy is a separate backup-first runbook (`CHS-Sprint-18-Remote-Deploy.md`) — **nothing flipped live.** Stripe test / notifications SIMULATE (incl. **push**) / `social_publish_mode=SIMULATE` / QBO sandbox / WC test sheet unchanged. **Cron count still 5** (push rides the existing `*/15` notification tick — no new trigger). No new server-side external integration.
...
**🟡 Open items for the S18 remote-deploy runbook / next session:** (1) the **full cross-module automated E2E** (lead→estimate→quote/deposit→job→notifications→photos→invoice→payment→completion→social) — the Opus-owned harness — was **scoped + the S18 slice verified live**, but the end-to-end automated harness across all prior modules is **not yet committed** (data-layer photo-lifecycle test added as a down payment). (2) Native iOS/Android build + store submission (Tony, per the checklist). (3) Optional `weekly_photo_summary` / `CurrencyInput` **not built** (deferred — no room-to-spare trigger met). (4) Module-Spec-Photo-Capture / -System-Admin / -Notifications docs referenced by the prompt are **not present in this repo** — built from the code seams per "trust the tree."

...
### Shipped — Sprint 18 (Jun 3, 2026) — Capacitor App & Polish ✅ DEPLOYED

**Tags:** `v0.18.0-sprint18` (code on `main`, commits `884ef12` feat + `be92096` heartbeat fix), `v0.18.0-sprint18-deployed` (production). **Worker version `b7a76418-87ad-466d-a447-4d5e0ee42d64`** on `https://chs-hub.tony-bc5.workers.dev` + `dashboard.homesolutionsar.com` + `client.homesolutionsar.com`. Migration `0039_device_tokens.sql` applied to remote by **direct execute** (not `migrations apply` — same unrecorded-ledger reason as prior sprints). Backup before remote writes: `backup_pre_sprint18_remote_20260603.sql` (local, gitignored) + R2 `chs-backups/`. Rollback tag if needed: `v0.17.0-sprint17-deployed` (`0039` creates a new table only — harmless under the old worker; no DB rollback needed). **Stripe test / notifications SIMULATE (incl. push) / `social_publish_mode=SIMULATE` / QBO sandbox / WC test sheet — nothing flipped. Cron count still 5.**

**Verified in production (curl + remote DB, Jun 3, 2026):** `/api/health/heartbeat` → `{"status":"ok","tables":41,"tables_expected":41,"settings":44}` (41 tables — `device_tokens` now monitored); `device_tokens` table present with both indexes, 0 rows (clean); `social_publish_mode` still `SIMULATE`; `/api/devices/register` returns 401 (not 404 — route wired, Cloudflare strips Cf-* headers as expected anti-spoof); `/api/devices/*` returns 404 on `client.homesolutionsar.com` (host guard intact); `/share/:token` still resolves on public host (410 for bad token, not 500); 5 cron schedules confirmed in deploy output; `NOTIFICATIONS_DISPATCH_MODE=simulate` in worker bindings.
...
**🟡 Open items carried forward:** (1) the **full cross-module automated E2E** (lead→estimate→quote/deposit→job→notifications→photos→invoice→payment→completion→social) — the Opus-owned harness — was scoped + the S18 slice verified live, but the end-to-end automated harness across all prior modules is **not yet committed** (data-layer photo-lifecycle test added as a down payment; run against prod as a follow-on Cursor task). (2) Native iOS/Android build + store submission (Tony, per `public/docs/CHS-Capacitor-Submission-Checklist.md`). (3) **⚠️ Pre-Launch blocker:** a native WebView won't carry a Cloudflare Access session to the Access-gated API — must resolve `APP_PUBLIC_ORIGIN`/public-hostname before the Capacitor app can authenticate. (4) Optional `weekly_photo_summary` / `CurrencyInput` not built (deferred). (5) Browser E2E pass for S18 features (Tony: Photos tab → annotate + save, before/after pair + slider, generate photo report + project packet, verify Documents tab) requires Tony's logged-in Access session and was not performed in this automated deploy.

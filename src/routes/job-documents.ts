...
export async function handleGenerateJobDocument(
  ...
  try {
    mergeFields = await resolveMergeFields(env, job, overrides, templateType as TemplateType);
  } catch (e) {
  ...
  const r2Key = TEMPLATE_R2_KEYS[templateType as TemplateType];
  console.log(`[job-documents] fetching template: key="${r2Key}" bucket=FILES`);
  ...
  if (!templateObj) {
    return err(500, "template_not_found", `R2 key: ${r2Key}`);
  }
  ...
  await env.DB.prepare(
    `INSERT INTO job_documents (id, job_id, template_type, filename, r2_key, generated_at, generated_by)
     VALUES (?, ?, ?, ?, ?, datetime('now'), ?)`,
  )
    .bind(docId, jobId, templateType, filename, generatedKey, user.email)
    .run();

  const downloadUrl = `/api/jobs/${jobId}/documents/${docId}/download`;

  return json(
    { id: docId, filename, r2_key: generatedKey, download_url: downloadUrl },
    { status: 201 },
  );
}

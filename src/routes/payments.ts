...
export async function handlePaymentCreate(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  const guarded = await guard(request, env, [...PAYMENT_ROLES]);
  if (guarded instanceof Response) return guarded;
  const { user } = guarded;
  ...
  // Conditional Lien Waiver is generated once via checkAndFireLienWaiver (called
  // from recordPayment when an invoice flips to paid + job is complete/fully paid).
  // Do NOT auto-gen per payment — that created duplicate waivers per payment row.

...
export async function handlePaymentCreate(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  // Sprint 20 — Auto-generate Conditional Lien Waiver on every client payment.
  // Non-blocking: lien waiver gen failure must never roll back the payment.
  const paymentDate =
  ...
  ctx.waitUntil(
    maybeAutoGenerateDocument(env, {
      ...
      relatedRecordId: result.paymentId,
      overrides: {
      ...
      console.error("[payments] doc auto-gen (lien_waiver_conditional) failed:", (e as Error).message),
    ),
  );

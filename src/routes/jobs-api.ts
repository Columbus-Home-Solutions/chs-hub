...
import {
  ...
  jobFinancialDeleteBlock,
} from "../lib/cascade-delete.js";
import { allocateNextJobNumber } from "../lib/number-counters.js";
...
export async function handleJobDelete(request: Request, env: Env, id: string): Promise<Response> {
  ...
  const blocked = await jobFinancialDeleteBlock(env, id, job.client_id);
  if (blocked) {
    return json({ error: "job_has_financials", message: blocked }, { status: 409 });
  }
  ...
  try {
    ...
    if (e instanceof CascadeStepError) {
      return json(
        {
